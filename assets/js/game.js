...
var Game = {
    ...
    update: function() {
        ...
        if (updateDelay % (10 - speed) == 0) {
            ...
            // (it has moved along with the rest of the snake).
            if(addNew) {
    ...
    appleCollision: function() {
        ...
        // This is needed ifthe apple spawns inside of the snake.
        for (var i = 0; i < snake.length; i++) {
            if (snake[i].x == apple.x && snake[i].y == apple.y) {
                // Next time the snake moves, the new block will be added to it length.
                addNew = true;
    ...
    wallCollision: function(head) {
        // Check if the head of the snake is in the boundaries of the game feild.
        if (head.x >= 600 || head.x < 0 || head.y >= 450 || head.y < 0) {
            // If it's not in , we've hit a wall. go to game over screeen.  
            game.state.start('Game_Over');
        }
    },  
